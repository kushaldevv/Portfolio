...
import { Link, Button, Box, Text, Image, Center, Wrap, WrapItem, Tag, TagLabel, useColorModeValue } from '@chakra-ui/react'
...
function Project({ title, desc, imagePath, techArray, githubLink, demoLink }) {
  let techArrayList = techArray.map((item, idx) => {
    return (<WrapItem key = {idx}>
      <Tag backgroundColor={useColorModeValue('black', 'white')} color={useColorModeValue('white', 'black')}>
        <TagLabel p='2' fontSize='md'>{item}</TagLabel>
      </Tag>
    </WrapItem>)
  })
  return (
    <Box>
      <Center className='project' overflow='hidden' backgroundColor='black' borderRadius='5%' transition='ease-in-out' position='relative' textAlign='center'>
        ...
        <Box className='project-text' px='2vw' position='absolute' top={'30%'}>
          ...
          <Text color='white' pt='3' fontWeight='medium' fontSize={['lg', '2xl', '2xl', '2xl']}>
            {desc}
          </Text>
        </Box>
      </Center>
